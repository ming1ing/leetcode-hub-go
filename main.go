...
XRLXXRRLX
题解：
1.容易想到比较每一个元素的数量，不相等返回false
2.一次移动操作指用一个"LX"替换一个"XL"，说明L是可以向左移
  同样用一个"XR"替换一个"RX"，R只能向右移
所以，如果start里面的L对应end里面L，出现右移，证明不存在移动的方法，返回false，同理，start的R出现左移，也是不合理。
时间复杂度:
O(len(start)+len(end))
空间复杂度:
O(len(start)+len(end))
...
func canTransform(start string, end string) bool {
	...
	// fmt.Println(idsl,idel)
	for i := 0; i < len(idel); i++ {
		if idsl[i] < idel[i] {
			return false
		}
	}
	// fmt.Println(idsr,ider)
	for i := 0; i < len(ider); i++ {
		if idsr[i] > ider[i] {
			return false
		}
	}
...
题解:
直接模拟，比较前后两个数，如果升序就加上当前元素与最大值比较，用于更新答案;如果降序，就更新子数组的和为当前元素
复杂度:
时间复杂度:O(n) n为数组长度
空间复杂度:O(1)
*/
